    ...
    - generic [ref=e362]:
      ...
      - generic [ref=e376]:
        - generic [ref=e377]:
          ...
          - generic [ref=e379]: "*"
        - textbox "Name" [ref=e383]: users Credential
        - generic [ref=e387]: "Character limit: 16/100"
      - generic [ref=e390]:
        - generic [ref=e391]:
      ...
      - generic [ref=e404]:
        - generic [ref=e405]:
          ...
          - generic [ref=e407]: "*"
          - link "Manual entry" [active] [ref=e409] [cursor=pointer]:
            - /url: "#"
        - textbox "Credential ID" [disabled] [ref=e413]: FIR691679
      - generic [ref=e416]:
        - generic "State/Province" [ref=e418]
        - generic "Open dropdown" [ref=e420]:
          - button "Select" [ref=e424]:
      ...
      - generic [ref=e442]:
        ...
        - spinbutton "Credential Cycle (Optional)" [ref=e448]
      ...
      - generic [ref=e455]:
        ...
        - generic "Open dropdown" [ref=e459]:
          - button "Select period" [ref=e463]:
            - generic [ref=e464]: Select period
      - generic [ref=e467]:
        - generic [ref=e471]:
        ...
        - generic [ref=e479]:
          ...
          - option "Do not automatically renew Do not automatically renew, it needs approval" [ref=e511] [cursor=pointer]:
            - generic [ref=e517]:
...
```ts
  ...
  4  | 
  5  | export class ToggleCredentialAutogenerateTask extends Task {
  6  |   private constructor(private readonly enabled: boolean) {
  7  |     super();
  8  |   }
  9  | 
  10 |   static on(): ToggleCredentialAutogenerateTask {
  11 |     return new ToggleCredentialAutogenerateTask(true);
  12 |   }
  13 | 
  14 |   static off(): ToggleCredentialAutogenerateTask {
  15 |     return new ToggleCredentialAutogenerateTask(false);
  16 |   }
  17 | 
  18 |   async performAs(actor: Actor): Promise<void> {
  19 |     const { page } = actor.abilityTo(BrowseTheWeb);
  20 |     const manualEntryLink = page.getByRole('link', { name: 'Manual entry' });
  21 |     const autogenerateLink = page.getByRole('link', { name: 'Autogenerate' });
  22 |     const idInput = page.locator('[data-testid="input_certification_id"]');
  23 | 
  24 |     if (this.enabled) {
  25 |       if (await manualEntryLink.isVisible()) {
  26 |         await manualEntryLink.click({ force: true });
  27 |         await autogenerateLink.waitFor({ state: 'visible', timeout: 10_000 });
  28 |         await autogenerateLink.click({ force: true });
  29 |       } else if (await autogenerateLink.isVisible()) {
  30 |         await autogenerateLink.click({ force: true });
  31 |       } else {
  32 |         const toggle = page.locator('[data-testid="auto-generate-xref-id"] .s-switch').first()
  33 |           .or(page.locator('[data-testid="auto-generate-xref-id"]'));
  34 |         if (await toggle.count()) {
  35 |           await toggle.click({ force: true });
  36 |         }
  37 |       }
  38 | 
  39 |       await page.waitForFunction(() => {
  40 |         const input = document.querySelector('[data-testid="input_certification_id"]') as HTMLInputElement | null;
  41 |         return Boolean(input?.disabled && input.value?.trim());
  42 |       }, { timeout: 15_000 });
  43 | 
  44 |       const currentId = (await idInput.inputValue()).trim();
  45 |       if (!currentId || /^FIR\d{1,4}$/i.test(currentId)) {
  46 |         if (await manualEntryLink.isVisible()) {
  47 |           await manualEntryLink.click({ force: true });
  48 |           await autogenerateLink.waitFor({ state: 'visible', timeout: 10_000 });
  49 |           await autogenerateLink.click({ force: true });
  50 |           await page.waitForFunction(() => {
  51 |             const input = document.querySelector('[data-testid="input_certification_id"]') as HTMLInputElement | null;
  52 |             return Boolean(input?.disabled && input.value?.trim() && !/^FIR\d{1,4}$/i.test(input.value.trim()));
  53 |           }, { timeout: 15_000 }).catch(() => undefined);
  54 |         }
  55 |       }
  56 |       return;
  57 |     }
  58 | 
  59 |     if (await manualEntryLink.isVisible()) {
  60 |       await manualEntryLink.click({ force: true });
  61 |     }
  62 | 
  63 |     await idInput.waitFor({ state: 'visible', timeout: 10_000 });
> 64 |     await page.waitForFunction(() => {
     |                ^ TimeoutError: page.waitForFunction: Timeout 10000ms exceeded.
  65 |       const input = document.querySelector('[data-testid="input_certification_id"]') as HTMLInputElement | null;
  66 |       return Boolean(input && !input.disabled);
  67 |     }, { timeout: 10_000 });
  68 |   }
  69 | }
  70 | 

...
- Name: credentials/C54133-verify-mandatory-field-validations-when-saving-credential-without-required-details.spec.ts >> Credentials >> @regression C54133: Verify mandatory field validations when saving a credential without required details
- Location: src/tests/credentials/C54133-verify-mandatory-field-validations-when-saving-credential-without-required-details.spec.ts:17:7
...
```ts
  ...
  16 | 
  17 |   test('@regression C54133: Verify mandatory field validations when saving a credential without required details', async ({
  18 |     admin,
  19 |   }) => {
  20 |     test.info().annotations.push(testrailCase(54133));
  21 | 
  22 |     await admin.attemptsTo(OpenAgencyCredentialsPageTask.page());
  23 |     expect(await admin.asks(IsAgencyCredentialsPageReadyQuestion.check())).toBe(true);
  24 | 
  25 |     await admin.attemptsTo(OpenAddAgencyCredentialModalTask.form());
  26 |     await admin.attemptsTo(SubmitBlankAgencyCredentialFormTask.form());
  27 | 
  28 |     const validation = await admin.asks(CredentialValidationDialogTextQuestion.displayed());
  29 |     // Dev70 may show inline field errors instead of the "Please fill the following" dialog.
  30 |     expect(validation).toMatch(
  31 |       /Please fill the following|Please set a name for the credential|Name \*|Character limit:\s*0\/100/i,
  32 |     );
  33 |     expect(validation).toMatch(/Please set an ID for the credential/i);
  34 |     expect(validation).toMatch(/Please select a credential type/i);
> 35 |     expect(validation).toMatch(/Name \*|Character limit: 0\/100/i);
     |                        ^ Error: expect(received).toMatch(expected)
  36 |     expect(await admin.asks(IsAddAgencyCredentialModalOpenQuestion.check())).toBe(true);
  37 |   });
  38 | });
  39 | 
```